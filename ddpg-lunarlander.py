    ...
    def select_action(self, state, noise=True):
        ...
        with torch.no_grad():
            if noise:
                ...
                action = self._actor_net(
                    torch.from_numpy(state).view(1, -1).to(self.device)
                )
...
def train(args, env: gym.Env, agent: DDPG, writer: SummaryWriter):
    ...
    for episode in range(args.episode):
        ...
        for t in itertools.count(start=1):
            ...
            if done:
                ...
                print(
                    "Step: {}\tEpisode: {}\tLength: {:3d}\tTotal reward: {:.2f}\tEwma reward: {:.2f}".format(
...
def test(args, env: gym.Env, agent: DDPG, writer: SummaryWriter):
    print("Start Testing")
